...
import { setUsers, setSelectedUser } from '../store/chat.store';
      ...
      <div className='flex flex-col gap-2 mt-4 transition-all duration-300'>
        {
          ...
          users.map((user,i)=>
          <div key={i}>
              <button  onClick={()=>{dispatch(setSelectedUser(user))}}  className={`flex flex-row items-center  gap-2 cursor-pointer hover:bg-gray-200 w-full rounded-lg p-2 ${user._id === selectedUser ? "bg-gray-200" : ""}`}>
